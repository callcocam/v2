                    ...
                    @foreach ($menuItems as $keyMenu => $menu)
                                ...
                                <ul x-collapse x-show="expanded">
                                    @foreach ($menu['submenu'] as $keyMenu => $submenu)
                                        <li
                                            ...
                                            <a @if (Route::has($submenu['route_name'])) href="{{ route($submenu['route_name']) }}" @else   href="" @endif
                                                class="flex items-center justify-between p-2 text-xs+ tracking-wide
                                                 ...
                                                 {{ $submenu['route_name'] === $pageName ? 'text-primary dark:text-accent-light font-medium' : 'text-slate-500 hover:text-slate-800 dark:text-navy-200 dark:hover:text-navy-50' }}">
                        ...
                        @else
                            <li @if ($menu['route_name'] === $pageName) x-init="$el.scrollIntoView({block:'center'});" @endif>
                                <a  @if (Route::has($menu['route_name'])) href="{{ route($menu['route_name']) }}" @else href="" @endif
                                    class="flex text-xs+ py-2  tracking-wide outline-none transition-colors duration-300 ease-in-out {{ $menu['route_name'] === $pageName ? 'text-primary dark:text-accent-light font-medium' : 'text-slate-500  hover:text-slate-800 dark:text-navy-200 dark:hover:text-navy-50' }}">
                                    {{ $menu['title'] }}
                                </a>
                            </li>
                        @endif
